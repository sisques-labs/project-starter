...
{
  private readonly logger = new Logger(SubscriptionActivatedEventHandler.name);
  ...
  /**
   * Handles the SubscriptionActivatedEvent event by updating the existing subscription view model.
   *
   * @param event - The SubscriptionActivatedEvent event to handle.
   */
  async handle(event: SubscriptionActivatedEvent) {
    this.logger.log(
      `Handling subscription activated event: ${event.aggregateId}`,
    );
