...
describe('PromptActivatedEventHandler', () => {
  let handler: PromptActivatedEventHandler;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<PromptActivatedEventHandler>(
      PromptActivatedEventHandler,
    );
  });
  ...
  describe('handle', () => {
    it('should update prompt view model status to ACTIVE', async () => {
      const aggregateId = '123e4567-e89b-12d3-a456-426614174000';
      const event = new PromptActivatedEvent(
        {
